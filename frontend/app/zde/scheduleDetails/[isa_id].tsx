...
const ScheduleDetailPage = ({ route }: any) => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      try {
        const res = await fetch(`http://172.20.10.2:5000/api/approvals/visitDetail/${isa_id}`);
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await fetch(`http://172.20.10.2:5000/api/approvals/zde/approve/${isa_id}`, {
        method: 'PATCH',
  ...
  const handleRequestRevision = async () => {
    ...
    setActionLoading(true);
    try {
      const res = await fetch(`http://172.20.10.2:5000/api/approvals/zde/reject/${isa_id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
